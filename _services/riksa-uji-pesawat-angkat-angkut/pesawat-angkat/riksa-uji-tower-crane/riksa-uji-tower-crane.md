---
layout: node/node--service
title: Riksa Uji Tower Crane
description: Riksa uji tower crane adalah pemeriksaan dan pengujian keselamatan crane
  menara oleh PJK3 resmi Kemenaker dalam rangka mendapatkan SIA/SILO/Suket K3 daripada
  Disnaker. Layanan pemeriksaan K3 semua Indonesia.
para_1: Riksa uji tower crane adalah inspeksi dan pengujian pada pesawat angkat jenis
  menara berdasarkan dengan cara Permenaker No. 8 Tahun 2020 perihal Keselamatan dan
  Kesehatan Kerja Pesawat Angkat dan Pesawat Angkut.
permalink: "/layanan/riksa-uji-pesawat-angkat-angkut/pesawat-angkat/riksa-uji-tower-crane/"
category: Riksa Uji Pesawat Angkat Angkut
sub_category: Pesawat Angkat
certification_category: pesawat-angkat
service_type: Safety Inspection
icon: bi-building-up
image:
  src: "/wp-content/uploads/2024/12/riksa-uji-tower-crane-pt-cipta-mas-jaya-001.jpg"
  alt: Riksa uji tower crane kuning di area proyek konstruksi yang dilakukan oleh
    tim inspector PJK3 PT Cipta Mas Jaya untuk memastikan keselamatan operasional
    sesuai standar Kemenaker dan regulasi K3 yang berlaku di Indonesia
images:
- src: "/wp-content/uploads/2024/12/riksa-uji-tower-crane-pt-cipta-mas-jaya-002.jpg"
  alt: Proses inspeksi tower crane dengan pekerja menggunakan APD lengkap helm safety
    di area konstruksi oleh tim inspector bersertifikat PT Cipta Mas Jaya sesuai standar
    keselamatan Permenaker No. 8 Tahun 2020
- src: "/wp-content/uploads/2024/12/riksa-uji-tower-crane-pt-cipta-mas-jaya-003.jpg"
  alt: Pemeriksaan kondisi tower crane di site proyek dengan material baja di sekitarnya
    oleh PJK3 resmi PT Cipta Mas Jaya untuk memastikan kelayakan operasi dan keamanan
    struktural crane menara
- src: "/wp-content/uploads/2024/12/riksa-uji-tower-crane-pt-cipta-mas-jaya-004.jpg"
  alt: Inspeksi detail slewing unit dan cabin operator tower crane untuk memastikan
    fungsi rotasi dan sistem kontrol bekerja dengan baik sesuai standar keselamatan
    K3 oleh teknisi inspector bersertifikat
area_served: Indonesia
content_structure: ''
date: 2025-12-07
last_modified_at: 2025-12-07
show_breadcrumb_schema: true
keywords: riksa uji tower crane, jasa riksa uji tower crane, inspeksi tower crane,
  crane menara, uji beban tower crane, PJK3, sertifikasi K3
rating_id: riksa-uji-tower-crane
custom_schema:
- schema/schema--service-reviews.html
lastmod: '2026-06-22T08:51:05+0000'
intro:
  name: Layanan Riksa Uji Tower Crane Profesional
  para: PT. Cipta Mas Jaya menyediakan layanan riksa uji tower crane secara cara cara
    cara inspector bersertifikat dan berpengalaman. Kami memverifikasi tower crane
    Anda memenuhi kriteria keselamatan yang ditetapkan oleh Kementerian Ketenagakerjaan
    RI.
  subsections:
  - h3: Dasar Hukum dan Standar Keselamatan
    para: Riksa uji tower crane wajib dijalankan sesuai regulasi keselamatan kerja
      yang berlaku terhadap Indonesia dalam rangka memverifikasi keamanan operasional
      terhadap proyek konstruksi.
    subsections:
    - h4: Permenaker No. 8 Tahun 2020
      para: Peraturan Menteri Ketenagakerjaan mengenai Keselamatan dan Kesehatan Kerja
        Pesawat Angkat dan Pesawat Angkut mewajibkan pengujian berkala dalam rangka
        segenap jenis crane meliputi tower crane.
    - h4: Standar FEM dan ISO
      para: Pengujian dijalankan mengacu pada acuan internasional FEM (Federation
        Europeenne de la Manutention) dan ISO 12480 dalam rangka crane safety dan
        Standar Nasional Indonesia (SNI) yang berlaku.
  - h3: Tim Inspector Bersertifikat
    para: Tim inspector kami memiliki sertifikat kompetensi daripada BNSP serta telah
      berpengalaman lebih daripada 10 tahun dalam bidang pengecekan pesawat angkat
      mencakup bermacam-macam jenis tower crane dalam rangka proyek konstruksi gedung
      serta infrastruktur.
  - h3: Cakupan Layanan Nasional
    para: Kami melayani riksa uji tower crane pada seluruh Indonesia, daripada proyek
      konstruksi gedung bertingkat, jembatan, sehingga infrastruktur besar. Tim kami
      siap datang ke lokasi proyek Anda kapanpun dibutuhkan.
jenis_alat:
  name: Jenis Tower Crane yang Kami Layani
  description: Kami melayani riksa uji untuk berbagai jenis tower crane konstruksi
  items:
  - name: Hammerhead Tower Crane
    description: Tower crane dengan jib horizontal dan counter-jib, jenis paling umum
      untuk konstruksi gedung bertingkat tinggi.
    icon: bi-building-up
  - name: Luffing Jib Tower Crane
    description: Tower crane dengan jib yang dapat diangkat naik-turun, ideal untuk
      area sempit dengan banyak crane berdekatan.
    icon: bi-arrow-up-right
  - name: Flat Top Tower Crane
    description: Tower crane tanpa cat head di bagian atas, memudahkan instalasi dan
      mengurangi risiko tabrakan antar crane.
    icon: bi-arrows-vertical
  - name: Self-Erecting Tower Crane
    description: Tower crane yang dapat merakit sendiri tanpa bantuan crane lain,
      cocok untuk proyek skala menengah.
    icon: bi-gear
  - name: Climbing Tower Crane
    description: Tower crane yang dapat naik mengikuti ketinggian bangunan dengan
      sistem climbing frame internal atau external.
    icon: bi-arrow-bar-up
  - name: Traveling Tower Crane
    description: Tower crane yang bergerak di atas rel untuk menjangkau area kerja
      yang lebih luas di proyek linear.
    icon: bi-train-front
  footer: Untuk jenis tower crane lainnya yang tidak tercantum di atas, silakan hubungi
    kami untuk konsultasi lebih lanjut.
komponen_inspeksi:
  name: Komponen Tower Crane yang Diperiksa
  description: Pemeriksaan menyeluruh pada seluruh bagian keselamatan tower crane
  items:
  - name: Mast dan Tower Section
    description: Pengecekan struktur menara, sambungan antar section, baut pengikat,
      serta kondisi las pada korosi maupun deformasi.
    icon: bi-building
  - name: Slewing Unit
    description: Pemeriksaan mekanisme putar termasuk slewing ring, motor slewing,
      serta sistem pengereman rotasi.
    icon: bi-arrow-repeat
  - name: Jib dan Counter-Jib
    description: Pemeriksaan kondisi lengan horizontal, sambungan, trolley rail, serta
      counterweight dalam rangka keseimbangan crane.
    icon: bi-arrows-expand
  - name: Hoisting Mechanism
    description: Pengecekan winch, drum, wire rope, hook block, serta sistem brake
      hoist dalam rangka memverifikasi fungsi pengangkatan aman.
    icon: bi-arrow-up-circle
  - name: Cabin dan Sistem Kontrol
    description: Pemeriksaan keadaan cabin operator, panel kontrol, joystick, display,
      dan sistem komunikasi.
    icon: bi-display
  - name: Safety Devices
    description: Tes load moment limiter, anemometer, anti-collision, travel limit
      switch, serta emergency stop.
    icon: bi-shield-check
proses_inspeksi:
  name: Proses Riksa Uji Tower Crane
  description: Tahapan inspeksi dan pengujian K3 tower crane oleh inspector bersertifikat
  totalTime: PT6H
  steps:
  - name: Pemeriksaan Dokumen
    text: Verifikasi dokumen teknis tower crane termasuk load chart, manual operasi,
      sertifikat sebelumnya, dan catatan pemeliharaan.
  - name: Inspeksi Visual Struktur
    text: Pemeriksaan kondisi fisik mast, jib, counter-jib, dan seluruh struktur untuk
      mendeteksi korosi, retak, atau deformasi.
  - name: Pemeriksaan Komponen Mekanis
    text: Inspeksi slewing mechanism, hoisting system, trolley, wire rope, hook, dan
      komponen mekanis lainnya.
  - name: Pengujian Sistem Elektrikal
    text: Pemeriksaan panel kontrol, motor, kabel, limit switch, dan sistem grounding
      untuk memastikan keamanan elektrikal.
  - name: Pengujian Safety Devices
    text: Testing load moment limiter (LMI), anti-collision system, anemometer, travel
      limits, dan emergency stop.
  - name: Load Test dan Dokumentasi
    text: Pengujian beban sesuai kapasitas crane dan penyusunan laporan inspeksi lengkap
      dengan penerbitan sertifikat kelayakan.
cara_pemesanan:
  name: Cara Pemesanan Riksa Uji Tower Crane
  description: Langkah-langkah mudah untuk memesan jasa riksa uji tower crane
  totalTime: P7D
  steps:
  - name: Hubungi Kami
    text: Hubungi tim kami via WhatsApp di 08568258841 atau email untuk konsultasi
      awal dan penawaran harga riksa uji tower crane.
  - name: Penjadwalan Inspeksi
    text: Tim kami akan mengatur jadwal inspeksi yang sesuai dengan timeline proyek
      Anda, biasanya dalam 3-7 hari kerja.
  - name: Inspeksi On-Site
    text: Inspector bersertifikat kami datang ke lokasi proyek untuk melakukan inspeksi
      menyeluruh sesuai standar K3.
  - name: Penerbitan Sertifikat
    text: Setelah tower crane lulus inspeksi, sertifikat kelayakan operasi diterbitkan
      dalam 3-5 hari kerja.
video_riksa_uji:
  name: Proses Riksa Uji Tower Crane - PT. Cipta Mas Jaya
  description: Video dokumentasi proses riksa uji tower crane oleh tim inspector bersertifikat
    PT. Cipta Mas Jaya.
  thumbnail: "/wp-content/uploads/2024/12/riksa-uji-tower-crane-pt-cipta-mas-jaya-001.jpg"
  src: ''
  duration: ''
  upload_date: ''
offer_riksa_uji:
  name: Paket Riksa Uji Tower Crane
  description: Layanan riksa uji dan inspeksi K3 tower crane dengan sertifikat resmi
  note: Harga belum termasuk biaya transportasi untuk lokasi di luar Jabodetabek.
    Harga dapat bervariasi tergantung ketinggian dan kapasitas tower crane.
  offers:
  - name: Riksa Uji Tower Crane Standar
    description: Inspeksi lengkap untuk 1 unit tower crane kapasitas hingga 8 ton
    price: 3500000
    price_currency: IDR
    price_label: per unit
    features:
    - Inspeksi visual struktur lengkap
    - Pemeriksaan komponen mekanis
    - Pengujian sistem elektrikal
    - Testing safety devices (LMI, limit switch)
    - Sertifikat resmi Disnaker
    - Laporan inspeksi detail
    availability: InStock
    valid_from: '2025-01-01'
    valid_through: '2025-12-31'
  - name: Riksa Uji Tower Crane Heavy Duty
    description: Inspeksi untuk tower crane kapasitas di atas 8 ton
    price: 4500000
    price_currency: IDR
    price_label: per unit
    features:
    - Inspeksi visual struktur komprehensif
    - Pemeriksaan komponen mekanis detail
    - Pengujian sistem elektrikal
    - Testing safety devices lengkap
    - Load test (jika diperlukan)
    - Sertifikat resmi Disnaker
    availability: InStock
    valid_from: '2025-01-01'
    valid_through: '2025-12-31'
  - name: Paket Riksa Uji Multi Unit
    description: Inspeksi untuk 3 unit tower crane atau lebih dengan harga khusus
    price: 3000000
    price_currency: IDR
    price_label: per unit (min. 3 unit)
    features:
    - Inspeksi visual struktur lengkap
    - Pemeriksaan komponen mekanis
    - Pengujian sistem elektrikal
    - Testing safety devices
    - Sertifikat resmi Disnaker
    availability: InStock
    valid_from: '2025-01-01'
    valid_through: '2025-12-31'
    is_featured: true
schedule_riksa_uji:
  name: Jadwal Riksa Uji Tersedia
  description: Pilih tanggal inspeksi yang sesuai dengan jadwal proyek Anda
  location: On-site di lokasi proyek Anda
  time_start: '08:00'
  time_end: '17:00'
  weeks_ahead_min: 1
  weeks_ahead_max: 6
  count: 4
  exclude_weekend: true
  note: Jadwal dapat disesuaikan dengan timeline proyek. Untuk inspeksi di luar jam
    kerja atau akhir pekan, hubungi kami untuk pengaturan khusus.
faq_riksa_uji:
  name: FAQ Riksa Uji Tower Crane
  description: Pertanyaan yang sering diajukan seputar layanan riksa uji tower crane
  faqs:
  - question: Apa itu riksa uji tower crane?
    answer: Riksa uji tower crane adalah pemeriksaan dan pengujian terhadap pesawat
      angkat jenis menara untuk memastikan kelaikan operasi dan keselamatan kerja
      sesuai Permenaker No. 8 Tahun 2020.
  - question: Berapa lama proses riksa uji tower crane?
    answer: Proses inspeksi on-site memakan waktu 4-6 jam tergantung ketinggian dan
      kondisi tower crane. Penerbitan sertifikat membutuhkan 3-5 hari kerja.
  - question: Apa saja yang diperiksa dalam riksa uji tower crane?
    answer: Inspeksi meliputi pemeriksaan struktur mast dan jib, slewing mechanism,
      hoisting system, wire rope, hook, cabin, sistem elektrikal, dan safety devices
      seperti LMI dan anti-collision.
  - question: Berapa biaya riksa uji tower crane?
    answer: Biaya riksa uji tower crane mulai dari Rp 5.500.000 per unit untuk kapasitas
      standar hingga 8 ton. Hubungi kami untuk penawaran khusus sesuai kebutuhan proyek
      Anda.
  - question: Apakah sertifikat riksa uji berlaku nasional?
    answer: Ya, sertifikat yang kami terbitkan diakui oleh Dinas Tenaga Kerja di seluruh
      Indonesia karena kami adalah PJK3 resmi berlisensi Kemenaker RI.
  - question: Berapa lama masa berlaku sertifikat riksa uji tower crane?
    answer: Masa berlaku sertifikat riksa uji tower crane adalah 1 tahun untuk crane
      operasional. Setelah itu, wajib dilakukan riksa uji ulang untuk memperpanjang
      izin operasi.
---
